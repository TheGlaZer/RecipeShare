...
const MyRecipes = ({ navigation }) => {
    ...
    const fetchMenus = async () => {
        ...
        const q = query(menuCollection, where('userId', '==', currentUser));
    ...
    const renderItem = ({ item }) => (
        <TouchableOpacity
            ...
            onPress={() => navigation.navigate('Menu', { ...item, edit: true })}
        >
            <MenuCard
                title={item.title}
                ...
                userId={item.userId}
                imageUrl={item.imageUrl} // Assuming you have an image URL in your Firestore data
            />
        </TouchableOpacity>
    );
    ...
    return (
        <Container>
            <Text style={{ fontSize: 46, fontWeight: '500' }}>My Recipes</Text>
            <FlatList
                data={menus}
